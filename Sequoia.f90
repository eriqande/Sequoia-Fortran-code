! Author: Jisca Huisman, post doc in Evolutionary biology at University of Edinburgh,
! jisca.huisman@gmail.com
! This code is available under GNU General Public License v3
!
!
! There are three programs:
! Duplicates - find duplicate genotypes and IDs in genotype file & lifehistory file,
! AssignParents - parentage assignment & list of non-assigned parent-offspring pairs
! Sibships - subsequent sibship clustering & grandparent assignment
! during compilation only one should be denoted as program and the others as subroutines.
! Future versions will probably have the other subroutines neatly bundled into modules.

! The program is described in the manuscript "Pedigree reconstruction from SNP data: 
! Parentage assignment, sibship clustering, and beyond", available on 
! https://github.com/JiscaH/Manuscripts-Papers  and submitted to Mol Ecol Resour Sept. 2016.

! Compile: gfortran -std=f95 -Og Sequoia.f90 -o Seq
! or: gfortran -std=f95 -Wall -pedantic -fbounds-check Sequoia.f90 -o Seq
...
nRounds, nC(2), nAgeClasses, nPairs, BY1, maxAgePO, mxA, XP !, DumBY1
...
! Adapted by J Huisman to output rank, in order to enable 
...
integer :: nDupGenoID, nDupLhID, nDupGenos, nSexless 
...
!print *, "dup: ", nDupGenoID, nDupLhID, nDupGenos, nSexless
...
subroutine AssignParents(nParents, nAmbiguous)
!program AssignParents
use qsort_c_module
use Global
implicit none

integer :: i, j, k, Round, nParents(2), nAmbiguous, isP(2), topX, maybe
...
call UpdateAllProbs
!print *, ""
!write(*, '("Initial total LL: ", f12.1)') SUM(Lind)
!print *, "" 
...
!print *, "done opposite homozygosity."
...
!    call intpr ( "round ",6, r, 1)
    ...
    call UpdateAllProbs
    print *, ""
    write(*, '("Total LL, parentage : ", i5, f12.1)') Round, SUM(Lind)
    print *, "no. parents assigned: ", COUNT(Parent(:,1)/=0), COUNT(Parent(:,2)/=0)
!    print *, "" 
    ...
    do i=1,nInd
        if (Sex(i)==3) then
            ...
            if (isP(1)>0 .and. isP(2)>0) then
                print *, "assigned as both dam & sire! ", ID(i), isP
            else
                do k=1,2
                    if (isP(k)>1) then  
                        Sex(i) = k
...
do i=1,nInd
    do k=1,2
        if (Parent(i,k)>0) then
            ...
            OppHomDF(i,k) = OppHomM(i, Parent(i,k))  ! note: otherwise parent=0 gives error
        endif
    enddo
enddo
...
integer :: Round, i, j, s, k, n, RX,  debug, LastR, topX
 character(len=1) :: RoundC
 character(len=2) :: RelName(8)
 character(len=4) :: DumTmp
 character(len=30), allocatable, dimension(:,:) :: PairName
...
do Round=1, Nrounds
 ...
 !   call intpr ( "Round: ",-1, Round, 1)
   ...
   write(*, '("Total LL IN: ", i5, f12.1)') Round, SUM(Lind)
    
    if (debug==0 .or. Round>1) then    
!    print *, " find pairs ..."
        ...
        print *, "Found Pairs: ", nPairs
        
        write(RoundC, '(i1)') Round
        open(unit=405, file="PairwiseLLR_"//RoundC//".txt", status="replace")
        write(405, '(2a30, 2a10, a15)') "ID1", "ID2", "PatMat", "dLLR"
        do i=1,nPairs
            write(405, '(2a30, i10, f15.4)') ID(PairID(i,1:2)), PairType(i),  PairDLLR(i)
        enddo
        close(405)
    else
        allocate(PairName(5*nInd,2)) 
        nPairs = 528
        open(unit=103, file="PairwiseLLR_1.txt", status="old")
        read(103,*)   ! header
        do i=1,nPairs
            read(103,*) PairName(i,1:2), PairType(i), PairDLLR(i)
        enddo
         close(103)
        do i=1,nPairs
           do k=1,2
               do j=1,nInd
                   if (PairName(i,k)== ID(j)) then
                       PairID(i,k) = j
                   endif
               enddo
           enddo
  ...
  !  call intpr ( "Clusters (pat): ",16, nC(2), 1)
    print *, "found clusters, maternal: ", nC(1), "paternal: ", nC(2)
    call UpdateAllProbs
    write(*, '("Total LL : ", f12.1)') SUM(Lind)
    ...
    if (nAgeClasses > 1 .and. (Round > RX .or. Round==Nrounds .or. LastR==1)) then  
        ...
        if (Round > RX+1) then
            print *, "grandparental pairs ..."
            call GGpairs
            call UpdateAllProbs
            write(*, '("Total LL : ", f12.1)') SUM(Lind)
        endif
        
        print *, "add grandparents ..."
        call SibGrandparents
        call UpdateAllProbs
        write(*, '("Total LL : ", f12.1)') SUM(Lind)
    endif
    
   write(*, '("Total LL OUT: ", i5, f12.1)') Round, SUM(Lind)
   print *, "with parent:", COUNT(Parent /=0, DIM=1)
   print *, ""
   print *, ""
 !   call dblepr("Total LL : ", -1, SUM(Lind), 1)
    ...
    if (Round == nRounds .or. LastR==1) then
        ...
        open (unit=407,file=trim(PedigreeFileName),status="replace")
        ...
        RelName = (/ "PO", "FS", "HS", "GP", "FA", "HA", "U ", "XX"/) 
!        call FindPairs(.FALSE.)  ! only finds likely HS
...
!        call WriteDummies  ! slow!   
        ...
        write (407,'(3a30)')  "ID", "Dam", "Sire"
        do i=1,nInd
            write (407,'(3a30,3f15.4)') Id(i), ParentName(i,1:2)
        enddo
        do k=1,2
            do s=1,nC(k)
                write (407,'(3a30,3f15.4)') DumName(s,k), GpName(:, s,k)
            enddo
        enddo
        close (407)
    endif
...
if (A==0 .and. B==350) then
    write(*, '("CalcPO ", 2a8, 2f9.2)') ID(A), ID(B), SUM(PrL), Lind(A)
endif
    ...
    if (ALL(nCP>0) .and. ANY(nCP>1)) then   ! test all combo's
        do u = 1, nCP(1)
            do v = 1, nCP(2)
                ...
                if (Parent(i, 2)/= CandPar(v,2))  call calcPOZ(i, CandPar(v,2), .FALSE.)   ! removes unlikely GPs
            enddo
        enddo
...
integer :: m, CurPar(2), TopX, k, x, CY(3), kY(3), y !, Inbr
double precision :: LLA(2,7,7), TopLL, LLcp(3), LLBA(7), TopBA, dLL, &
  LLtmp(2), LLINOUT(2), ALR(3)

 CurPar = Parent(A,:)
if (A==1136 .and. B==1261) then
    print *, ""
    print *, "=================="
    write(*, '("POZ ", 2a8, i4, " , CurPar: ", 2i5, " ", f8.2, 2i5)') ID(A), ID(B), AgeDiff(A,B), Parent(A,:), Lind(A), Parent(B,:)
endif
...
!    if (nS(-Parent(A,3-k),3-k)==2) then
!        Inbr = 1  ! current pair of HS
!    endif
!endif
...
LLA = 999
LLBA = 999
if ((Parent(A,1)==0 .and. Parent(A,2)==0)) then  !  .or. Inbr==1
    ...
    if (AgeDiff(A,B)==999) then
        ...
        if (Parent(B,m)==0) then
            ...
            TopBA = MaxLL(LLBA)  
            if ((A==0 .and. B==1461) .or. (A==0 .and. B==1743)) then
                write(*,'(2a8)') ID(A), ID(B)
                write(*,'("LLA: ", 7f8.1)') LLA(1,:,7)
                write(*,'("LLBA: ", f8.1, "; ", 7f8.1)') TopBA, LLBA
                write(*,'("LLtmp: ", 2f8.1)')  LLtmp(1:2)
            endif
    ...
    do m=1,2
        if (CurPar(m)==0) cycle  ! LLA(m,:,:) empty if only 1 CurPar  !  .and. Sex(B)/=3
...
if ((LLINOUT(1) - LLINOUT(2) > 5) .or. (A==1136 .and. B==1261)) then
    if (LLINOUT(1) - LLINOUT(2) > 5) print *, "Decrease in LL! ", A, ID(A), ID(B), k
    write (*, '("Parents: ", 2i5, f8.1, " -> ", 2i5, f8.1)') curPar, &
      LLINOUT(1), Parent(A,:), LLINOUT(2)
    do m=1,2
       print *, "m=",m
       do x=1,7
           write(*,'(7f8.1)') LLA(m,x,:)
      enddo
   enddo
   write(*,'("topLL: ", f9.2, "  parents: ", 2i6, ", ", 3f8.1)') topLL, Parent(A,:), LR_parent(A,:)
   write (*, '("ALR: ", 3f9.2, ", LLtmp: ", 2f9.2)') ALR, LLtmp
   print *, ""
!    if (LLINOUT(1) - LLINOUT(2) > 5)  stop
endif
...
!        call intpr ( " ",1, i, 1)
    ...
    do j=i+1,nInd
        do k=1,2
            ...
            if (i==727 .and. j==0) then
                write(*,'("pair? ", 2a8, i3, 2f9.2)') ID(i), ID(j), k, LRS
            endif
            if (AgeDiff(i,j)==999 .or. AgeDiff(i,j)>=0) then
            ...
            if (i==0) then
                write(*,'("pair? ", 2a8, i3, f9.2, "; ", 7f8.1)') ID(i), ID(j), top,dLL, tmpLL 
            endif
            if (top==2 .or. top==3) then  ! shared top
...
!                endif
                ...
                if (i==0 .and. j==172) then
                    write(*,'("pair ", i7, 2i5, " ", f8.1, i5)') nPairs, PairID(nPairs, :), PairDLLR(nPairs), PairType
                endif
            endif
        enddo
    enddo
...
!print *, "Pairs found: ", nPairs
!print *, "maternal: ", COUNT(PairType==1), "  , paternal: ", COUNT(PairType==2)
!print *, ""
...
!    if (MaxLL(LL)==LL(2)) then  ! check if HS via not-yet-assigned parent
!        if (Parent(A,3-k)==0 .and. Parent(B,3-k)>0) then
!            call PairPO(A, Parent(B,3-k), 3-k, LLX(1))
!            call CalcU(A, 3-k, Parent(B,3-k), 3-k, LLX(2))
!        else if (Parent(B,3-k)==0 .and. Parent(A,3-k)>0) then
!            call PairPO(B, Parent(A,3-k), 3-k, LLX(1))
!           call CalcU(B, 3-k, Parent(A,3-k), 3-k, LLX(2))
!       endif
!       if (LLX(1) - LLX(2) > thLR) then
!           LL(2:3) = 222
!      endif
!    endif
...
!if (((A==1022 .or. A==1021) .and. B==994) .or. (A==994 .and. (B==1022 .or. B==1021)) .or. &
if ((A==1136 .and. B==1261) .or. (A==0 .and. B==964)) then
!if (k==0 .and. A>373 .and. A<378 .and. B>373 .and. B<378) then
    print *, ""
    write (*, '("pair?", i3, " ",a8, 2i6, " ",a8, 2i5, 7f8.1)') k, ID(A),Parent(A,:), ID(B), Parent(B,:), LL
    write (*, '("LLG ", 7f8.1)') LLG
    write (*, '("LLX ", 4f8.1, ", ", 2f8.1)') LLX, LLZ
    write (*, '("LLUA ", 3f8.1, "; ", 3f8.1, "; ", 2f8.1)') LLtmpA(1,:), LLtmpA(2,:),LLtmpGGP, LLCC 
    call CalcU(B,3-k,A,3-k, LLg(7))    
    write (*, '("LU ", f8.1, "; ", 3f8.1)') LLg(7), Lind(A) + Lind(B), Lind(A), Lind(B)  
    print *, ""
endif
...
if (A==1136 .and. B==1261) then
    write (*,'("PairPO ", 2a8, 3f8.1, "; ", f8.1)') ID(A), ID(B), LLtmp, LL
endif
...
if (A==0 .and. B==160) then
    write(*,'("pairFS ", 2a8, 2i5, 3f8.1)') ID(A), ID(B), Par  !, LLtmp, LLU   
!    write(*,'("LLtmp ", f8.1, " cor: ", 4f8.1)') LLtmp, CLL(-par(1),1) + Lind(A) + Lind(B), CLL(-par(1),1),Lind(A),Lind(B)
endif
...
if (A==1116 .and. B==0) then
    write(*,'("pairHS ", 2a8, 2i5, f8.1)') ID(A), ID(B), Par, Inbr, LL  
endif
...
! calculates LL that B is maternal(k=1) or paternal(k=2) grandparent of A
use Global
implicit none
...
do l=1,nSnp
    ...
    PrXZ = 1
    do x=1,3  ! PA(k)
        if (Parent(A,k)/=0) then   ! ideally not called for Parent(A,k)>0
            PrXZ(x,:,:,:) = PrPA(x, k)
        endif
        do y=1,3  ! PA(3-k)
            do z=1,3  !  PrG(3-m)
...
if (A==1116 .and. B==0) then
    write(*, '("pairGP", i5, " ", 2a8, 3f10.1, "; ", 3f8.1)') k, ID(A), ID(B), &
      SUM(PrL, DIM=1), Lind(B), LLtmp
    print *, Parent(A,:), ", ", curGP, ", ", Parent(B,:)
!    write(*, '(f8.1)') SUM(PrL(:,2))-LUX+Lind(A)
    print *, ""
endif
...
if (A==0 .and. B==389) then
    write(*, '("PairGGP ", 2a6,  2f8.1, "; ", 3f8.1)') ID(A), ID(B), Lind(A), Lind(B), SUM(PrL, DIM=1)
    if (Parent(A,3-k) < 0) then
!        write(*, '(3f8.1)')  LUX, CLL(-Parent(A,3-k), 3-k), LLtmp(3)
    endif
    print *, ""
endif 
...
if (kB==3 .and. B < 0) then 
    print *, "UA for kB=3 and SB not implemented"
endif
...
if (kA/=kB .and. kB/=3) then
    ...
    enddo
!if (kA==kB .and. B<0 .and. GA/=0) then
!    if (GPID(3-kB, -B, kB) == GA) then
!    
!    endif
endif
if (kB/=3) then
...
if (A==-36 .and. kA==2 .and. B==141 .and. kB==0)  then
!    if (GpID(1,-A,kA)/=0 .or. GpID(2,-A,kA)/=1275)  return
    write(*,'("UA: ", 2i5, ", ",2i5, ", ", 2i5, f9.2, 2i3)') kA, A, kB,B, GG, LL, cat(nA+1), catG
    do i=1, nA
        print *, ID(AA(i)), Parent(AA(i), :), ", ", cat(i)
    enddo
    print *, ""
 !   do i=1, nB
 !      print *, ID(BB(i)), Parent(BB(i), :), ", ", DoneB(i), catB(i)
 !   enddo
 !   print *, "Par GG: ", Parent(GG(kB), :), " , GA: ", GA
 !   write(*,'("BBx: ", 2i3, ", ",20i5)') nBx, BBx(1:nBx(kB), kB)
!    write(*, '("AncA1: ", 8i6)') AncA(1, 1:8)
!    write(*, '("AncA2: ", 8i6)') AncA(2, 1:8)
    print *, ""
!    if (LL > 0)  stop
endif 
...
!        if (R==1 .and. PK==3 .and. k==2) cycle
        ...
        if (sx(1)==0 .and. sx(2)==0) then
            ...
            if (ij(1)==0 .and. ij(2)==377) then
                write(*,'("NP? ", 2i3, " ", 2a8, f9.2, i3, "; ",7f8.1)') k, PK, ID(ij), dLL, topX, LLtmp    
            endif
            ...
            if (topX==2 .or. (topX==3 .and. (dLL > 2*thLRrel .or. LR>=0))) then
                ...
                enddo
                call CalcCLL(nC(k), k)                 
                if (ij(1)==0 .and. ij(2)==1513) then
                   write(*,'("NP ", i3, " ", 2a8, f8.1, " , ", 7f8.1)') k, ID(ij), CLL(nc(k),k), LLtmp
               endif
            endif
        else if (sx(1)>0 .and. sx(2)>0 .and. sx(1) /= sx(2)) then   ! else do nothing
            call CheckMerge(sx(1), sx(2), k,k, LLtmp, 1)
            call BestRel(LLtmp, 3, topX, dLL)  
            if (ij(1)==0 .and. ij(2)==841) then
                write(*,'("merge? ", i3, " ", 2a8, 2f8.1, 2i4, "  ", 7f8.1)') k, ID(ij), CLL(sx(1),k), CLL(sx(2), k), sx, LLtmp
            endif
            if (topX==1 .and. dLL > thLRrel * MIN(nS(sx(1),k), nS(sx(2),k))) then   ! (nS(sx(1),k) + nS(sx(2),k))/2   
            ...
            do m=1,2
                if (sx(m)>0 .and. sx(3-m)==0) then
                    ...
                    call BestRel(LLtmp, 3, topX, dLL)
                    if (ij(1)==0) then
!                    if (ij(1)>450 .and. ij(1)<456 .and. ij(2)>450 .and. ij(2)<456) then
                       write(*,'("add? ", i3, " ", 2a8, 2f8.1, 2i4, 7f8.1)') k, ID(ij), Lind(ij(3-m)), CLL(sx(m), k), &
                         sx(m), nS(sx(m), m), LLtmp
                    endif
...
double precision :: LLtmp(7,2), LLm(7), dLL !, LLINOUT(2)
...
!            if (SibID(1,s,k)==128 .and. SibID(1,r,k)==139) then
            if (k==1 .and. ANY(SibID(:,s,k)==689) .and. topX==0) then
                write(*, '("merge? ", 3i5," ",2a8, 7f8.1)') k, s, r, ID(SibID(1,s,k)), ID(SibID(1,r,k)), LLm  
            endif            
            if (topX==1 .and. dLL > thLRrel * MIN(nS(s,k),nS(r,k))) then
!                write(*, '("merge ", 3i5," ",7f8.1, 2i5, f9.2, " ",2a8)') k, s, r, LLm, &
!                  nS(s,k), nS(r,k), dLL, ID(SibID(1,s,k)), ID(SibID(1,r,k))
!                print *, ID(SibID(1:nS(s,k),s,k))
!                print *, ID(SibID(1:nS(r,k),r,k))
!                print *, ""

!                LLINOUT(1) = SUM(Lind(SibID(1:nS(s,k),s,k))) + SUM(Lind(SibID(1:nS(r,k),r,k)))
                call DoMerge(s, r, k)
!                LLINOUT(2) = SUM(Lind(SibID(1:nS(s,k),s,k)))
!                if (LLINOUT(1) > LLINOUT(2)) then
!                    write(*,'("decrease in LL!", 3i3, 2f8.1, 2i4)') k,s,r, LLINOUT, nS(s,k), nS(r,k)
!                endif
...
! TODO: consider change of (dummy)parent
use Global
implicit none
...
do i=1, nInd
    ...
    do k=1,2
        ...
        do s=1,nC(k)  ! sibships
            ...
            call BestRel(LLtmp, 3, topX, dLL)
            if (k==0 .and. s==45 .and. i==550) then
                write(*,'("grow? ", a8, 3i5, " ", a8, f9.2, "; ", 7f8.1)') ID(i), k, s, nS(s,k), &
                  ID(SibID(1,s,k)), dLL, LLtmp
            endif
            if (topX==2 .or. topX==3) then   !   * (nS(s,k)+2)/2  ! *nS(s,k)
                nMaybe = nMaybe+1
                Clm(nMaybe) = s 
                dLLM(nMaybe) = dLL
            endif
...
!            print *, "2 maybe sibships: ", ID(i), k, ", ", Clm(1:nMaybe)
!           call CheckMerge(Clm(1), Clm(2), k, k, LLtmp, 0)
!            write(*,'("merge? ", 7f8.1)') LLtmp   ! TODO.
!            print *, ""
!        else if (nMaybe>2) then ! TODO: allow 1 of one type & =MAX, >1 of other
!            print *, ">1 maybe sibship: ", ID(i), k, ", ", Clm(1:nMaybe)
        endif
    enddo
enddo
...
integer :: k, s, xs, i, n,maybe, topX, CurNumC(2),  &  ! nCandPar,
...
!                if (i==600 .and. s==22 .and. k==2) print *, "SibPar OH: ", ID(i), ID(SibID(n,s,k)), OH
...
!                if (i==600 .and. s==22 .and. k==2) print *, "SibPar (G) OH: ", ID(i), ID(GpID(n,s,k)), OH
                if (OH > maxOppHom) then
                    maybe = 0
                    exit
                endif   
            ...
            if (Sex(i)==3) then  ! check if parent of opposite sex instead
                ...
                if (MaybeOpp == 1) then
                    if (Par < 0) then  ! may have more/fewer sibs
                        ...
                        call PairPO(SibID(1, s, k), i, 3-k, LLtmp(1))   ! TODO: with all sibs?
...
do i=1, nInd
    ...
    do j=1, nInd  ! candidate parent.
        ...
        call CalcPO(i, j, LRP)
        if (i==3013 .and. j==0) then
            write(*, '("MorePar ", 2a8, f9.2)') ID(i), ID(j), LRP
        endif
        if (LRP < -thLR) cycle 
    ...
    do xs=1, nC(k)
        ...
        do i=1,nInd
            ...
            if (nS(s,k)>1) then
                ...
                if ((s==39 .or. s==38) .and. k==0 .and. i==138) then
                    print *, ""
                    write(*,'("GP? ", 2i4," ", i6, f8.1)') k, s, i, LRG
                endif
        ...
        do m=1,2
            do r=1, nC(m) 
                ...
                enddo  
                if (k==0 .and. m==2 .and. ANY(SibID(:,s,k)==5102)) then  ! 
                    write(*,'("GP? ", 4i5, f8.1)') k,s,m,r, LRG
!                    write(*, '("AncR1", 32i6)') AncR(1, :)
!                    write(*, '("AncR2", 32i6)') AncR(2, :)
           !         print *, "nFS ", COUNT(nFS(SibID(1:nS(r,m),r,m))>0), "; ", nFS(SibID(1:nS(r,m),r,m))
                endif 
...
!            do m=1,2  ! opportunity to restore previous GPs
!                if (GpID(m,s,k) == CurGP(m) .or. CurGP(m)==0) cycle
!                call CalcGPz(s, k, CurGP(m), m) 
!            enddo
!        if (CLL(s,k) - curCL < -thLRrel) then  ! made worse somehow.
!            write(*,'("decrease in GP, resetting ", 2i5, ", ", 2i5, " <- ", 2i5, 2f8.1)') s, k, &
!              curGP, GpID(:,s,k), curCL, CLL(s,k)
!            GpID(:,s,k) = CurGP
!            call CalcCLL(s,k)
!        endif
...
!if (kA==1 .and. ANY(SibID(:,SA,kA)==5102) .and. B==156) then
if (kA==1 .and. SA==31 .and. B==1025) then
    print *, "====================="
    if (B>0) then
        write(*,'(a8, i3, i6, " GPs: ", 2i6, ", ", 2f8.1)') ID(B), kA, SA, GpID(:,SA,kA), CLL(SA,kA), Lind(B)
    else
        write(*,'(4i5, " GPs: ", 2i6, ", ", 2f8.1)') kB,B, kA,SA, GpID(:,SA,kA), CLL(SA,kA), CLL(-B,kB)
    endif
endif
...
if (curGP(1)==0 .and. curGP(2)==0) then
 ...
 !       GpID(n,SA,kA) = 0
!        if (LLA(3-n,4,4)==TopLL) then
!            call BestRel(LLA(3-n,:,4), 4, topX, dLL)  
!        else
!            call BestRel(LLA(n,:,7), 4, topX, dLL)  
!        endif
...
LLINOUT(2) = CLL(SA,kA)
 ...
 if ((LLINOUT(1) - LLINOUT(2)) > 20 .or. (kA==1 .and. SA==31 .and. B==1025)) then
     if ((LLINOUT(1) - LLINOUT(2)) > 20) then
        write(*, '("Decrease in CLL!! ", 4i5, " : ", 2f9.2)') SA, kA, B, kB, LLINOUT
        print *, "GPs", curGP, " -> ", GPID(:, SA, kA)
    endif
!    write(*,'(1f8.1, " -> ", f8.1)') curCLL, CLL(SA,kA)
!    print *, ""
! endif
 
!if (kA==1 .and. SA==87 .and. B==773) then
    do x=1,2
        write(*,'("LLCP: ", 3f8.1)') LLCP(:,x)
    enddo
    write(*,'("LL U: ", 3f8.1, ", I/O: ", 2f8.1)') LLU, LLINOUT
    if (B < 0) then
        print *, "GB: ", GpID(:, -B, kB)
    endif
   do m=1,2
   ...
   enddo
   print *, ""
   write(*,'("topLL: ", 2f9.2, "  GPs: ", 2i6)') topLL,  MaxLL(RESHAPE(LLA(:,:,4), (/2*7/))),  GpID(:,SA,kA)
   print *, ""
!   if ((LLINOUT(1) - LLINOUT(2)) > 20) stop
!    if (kA==2 .and. SA==22 .and. B==4429) stop
endif
...
do i=1, nInd
    ...
    do k=1,2
        ...
        do j=1, nInd
            ...
            call PairQHS(i, j, LRS)  ! quick check
            if (i==0 .and. j==166) then
                write(*, '("GGpair  ", 2a8, f8.1)') ID(i), ID(j), LRS
            else
                if (LRS < -thLR) cycle
            endif
 ...
 !               if (i==2319 .or. i==5410 .or. i==5111) then
 !                   write(*, '("GGpair  ", 2a8, i3, 7f8.1)') ID(i), ID(j), Sex(j), LLtmp
 !               endif
                nC(k) = nC(k) + 1
                nS(nC(k),k) = 1
                SibID(1, nC(k), k) = i
...
if (A==529 .and. k==0 .and. ANY(SibID(1:nS(SB,k),SB,k)==530)) then
    write(*,'("LRHS ", a8, 2i5, f9.2, 2i4)') ID(A), k, SB, LRHS  !, focal, Sex(A)
 endif
...
if (ANY(LL > HUGE(0.0D0))) then
    print *, ""
    print *, "add LL = INF!", A, SB, k, focal
    write(*,'("LL: ", 7f8.1)') LL
    print *, ""
    stop
endif   

!if ((A==934 .and. k==1 .and. SB==9) .or. ((A==3581 .or. A==1265) .and. k==1 .and. SibID(1,SB,k)==5111) .or. &
!  ((A==1112 .or. A==1020) .and. k==1 .and. SB==99)) then  
if (A==0 .and. k==2 .and. SB==10) then 
!if (A==1551 .and. k==1 .and. ANY(SibID(1:nS(SB,k),SB,k)==1386)) then  
    print *, ""
    write (*, '("add?", i3, " ", a6, 2i5, " ;", 3i5, ", ", 7f8.1)') k, ID(A), Parent(A,:), SB, GpID(:,SB,k), LL
    write (*, '("LRHS", f8.1, " , LLg ", 7f8.1)') LRHS, LLg
    write (*, '("LLua ", 4f8.1, "; ", 5f8.1)') LLtmp(1,:), LLtmp(2,:), LLz
    write (*, '("ALR ", 7f8.1, "; ", 4f8.1)') ALR, ALRx(1,:), ALRx(2,:)
!    write (*, '(30a8)') ID(SibID(1:nS(SB,k),SB,k))
    write (*, '("LLU ", f8.1, "; ", 3f8.1)') LLg(7), Lind(A) + CLL(SB,k), Lind(A), CLL(SB,k) 
    write (*, '("LLM ", 2i5, "; ", 3f8.1)') MaybeOpp, Par, LLM
    do x=1, nS(SB, k)
        write(*,'(i3, " ",a8, 2i6, " fs", 10i5)') SB, ID(SibID(x,SB,k)), Parent(SibID(x,SB,k),:), &
...
!    if (SibID(1,sA,kA)==3457 .and. kA==1 .and. kB==1 .and. SibID(1,SB,kB)==3542) then 
!        write(*,'("LRHS ", f9.2)') LRHS
!    else
       if (LL(1) == 777 .and. focal==1) return
!    endif
endif
...
!if (kA==2 .and. ((ANY(SibID(:,sA,kA)==487) .and. ANY(SibID(:,SB,kB)==488))  .or. &
!  (ANY(SibID(:,sA,kA)==488) .and. ANY(SibID(:,SB,kB)==487)))) then 
!    print *, "yep"
!else
!  if (focal==1 .and. (LL(1)==777 .or. LL(1) < LL(7))) return
!endif
...
!            if (SA==6 .and. kA==1 .and. kB==2 .and. SB==32) then
!                write(*, '("chk ", a8, i5, 3f8.1)') ID(SibID(j, SB, kB)), par(1), LLHA
!            endif
        endif
    enddo
...
!            endif
 ...
 call CalcU(-SB,kB, -SA,kA, LUX)
!if (ABS(LL(7) - LUX) > thLRrel) then 
!if (kA==0 .and. kB==0 .and. SA==5 .and. SB==24) then
if (kA==0 .and. kB==1 .and. ((ANY(SibID(:,sA,kA)==10) .and. ANY(SibID(:,SB,kB)==1015))  .or. &
  (ANY(SibID(:,sA,kA)==1015) .and. ANY(SibID(:,SB,kB)==10)))) then 
    print *, ""
    write(*,'("merge? ", 2i5," ,",2i5,": ", 7f8.1)') kA, SA, kB, SB, LL
    write(*,'("LLtmp, LLx ", 2f8.1, "; ", 6f8.1, "; ", 2f8.1)') LLtmp, LLx, LLz
    write(*,'("LLM ", 6f8.1, " ; ", 2f8.1)') LLM, LLM(2)-LL(7), LLM(3)-LLM(1)
!    print *, "par: ", Par, " , nS: ", nS(-Par(1), 3-kA), nS(-Par(2), 3-kB)
    if (par(1)<0 .and. par(2)<0) then
        print *, "nS par: ", nS(-par, 3-kA)
    endif
    write(*,'("ALR ", 20f8.1)') ALR
    write(*,'("ALRx ", 20f8.1)') ALRx
    write(*,'("GA: ", 2i5," , GB: ", 2i5)') GpID(:, SA, kA), GpID(:, SB, kB)
    write(*,'("LU2: ", 2f8.1, " , ", 2f8.1)') LUX, CLL(SA, kA) + CLL(SB, kB), CLL(SA, kA), CLL(SB, kB)
    write(*,'("dLH ", 20f8.1)') dLH
    do i=1, nS(SA, kA)
        write(*,'(i3, " ", a8, 2i6, " fs", 10i5)') SA, ID(SibID(i,SA,kA)), Parent(SibID(i,SA,kA),:), &
          nFS(SibID(i,SA,kA)), FSID(1:nFS(SibID(i,SA,kA)), SibID(i,SA,kA))
    enddo
    print *, ""
    do i=1, nS(SB, kB)
        write(*,'(i3, " ",a8, 2i6, " fs", 10i5)') SB, ID(SibID(i,SB,kB)), Parent(SibID(i,SB,kB),:), &
          nFS(SibID(i,SB,kB)), FSID(1:nFS(SibID(i,SB,kB)), SibID(i,SB,kB))
    enddo
!   print *, ID(SibID(1:nS(SA,kA), SA, kA))
!   print *, ID(SibID(1:nS(SB,kB), SB, kB))
    print *, ""
!    stop
endif
...
if (SA==7 .and. SB==10 .and. k==0) then
    write(*,'("FSmerge ", 3i5, " - ", 2i5, "  ", 4f9.2)') k, SA, nS(SA,k), SB, nS(SB,k), LL
endif
...
if (nS(SB,k) +1 >= maxSibSize) then
    print *, "DoAdd: reached maxSibSize ", k, A, SB
    stop
endif
...
if (SA/=0) then
    if (nS(SA,k) + nS(SB,k) >= maxSibSize) then
        print *, "DoMerge: reached maxSibSize ", k, SA, SB
        stop
    endif
...
if (A==0) then
    ...
    if (Parent(A,kB)==B) then
        ...
        return
    else if (ALL(Parent(A,:)>=0)) then  ! .or. ALL(GpID(:,-B,kB)>=0)
        LL = Lind(A) + CLL(-B, kB)
        return
    else
    ...
    if (Parent(B,kA)==A) then
        ...
        LL = CLL(-A,kA) + Lind(B)  
        return
    else if (ALL(Parent(B,:)>=0)) then  ! .or.  ALL(GpID(:,-A,kA)>=0) .and.
        LL = CLL(-A, kA) + Lind(B) 
        return
    else
...
if (A>0 .and. B>0) then  ! == pairs ==
    ...
    if (cat==0 .or. cat==5) then  ! TODO? cat=5
        ...
        kB = 2
        cat=0
!        call UClust(par(1), par(2), 1, 2, 0, A, B, LL)  (below)
    else
...
if (kA==0 .or. kA>2) then
    print *, "calcU error! SA and/or SB 0: ", A, kA, SA, Ai, "; ", B, kB, SB, Bj
!if (A==4429 .and. kBIN==2 .and.  B==-22) then
!    if (GpID(1,-B,kB)/=3245 .or. GpID(2,-B,kB)/=0) return !!
    print *, "CalcU: ", cat, swap
    if (A>0) print *, "parA: ", Parent(A,:)
    if (A<0) print *, "GA: ", GpID(:, -A,kA)
    if (B>0) print *, "parB: ", Parent(B,:)
    if (B<0) print *, "GB: ", GpID(:, -B,kB)
    stop
endif
...
if (A==1549 .and. kBIN==1 .and.  B==-20) then
!    if (GpID(1,-B,kB)/=3245 .or. GpID(2,-B,kB)/=0) return !!
    write(*,'("CalcU ", 4i5, f9.2, 3i5)')  A, kA, B, kB, LL, cat, Ai, Bj
    print *, SA, SB, swap
    print *, ""
endif
...
do l=1, nSnp
    ...
    if (cat==2) then
        if (Par(1)<0 .and. Par(2)<0) then  ! SHOULD NOT  BE CALLED.
            print *, "Upair called erroneously", A, B, cat
            stop
        else
            do m=1,2
...
if (A==1778 .and. B==216) then
    write(*,'("Upair: ", 2a8, i5, f8.1)') ID(A), ID(B), cat, LL
endif
...
if (B==-20 .and. kB==1 .and.  Ai==1549) then
!    if (Parent(A,2)==0) return  !!!
    print *, ""
    write(*,'("Uclust ", 4i5, f9.2, 3i5)')  A, kA, B, kB, LL, cat, Ai, Bj
    write (*, '(3i5, " , catA ", 30i3)') A, GA, catA(1:nA)
    write (*, '(3i5, " , catB ", 30i3)') B, GB, catB(1:nB)
!    print *, "DoneA ", DoneA(1:nA)   
    do i=1,nA   
        write(*,'(a8, " ", 4i5, " ", f8.1, " ", i4)') ID(AA(i)), Parent(AA(i),:), &
          nFS(AA(i)), catA(i), Lind(AA(i)), DoneA(i)  
    enddo
    print *, ""
    do i=1,nB   
        write(*,'(a8, " ", 4i5, " ", f8.1)') ID(BB(i)), Parent(BB(i),:), nFS(BB(i)), catB(i), Lind(BB(i))  
    enddo
    print *, ""
!    write(*,'(a8, " ", f8.1)') ID(PA), Lind(PA)
endif

!if (LL > HUGE(0.0D0)) then
if (A<0 .and. B<0) then
   if ((CLL(-A,kA) + CLL(-B, kB)) - LL > 500) then  ! negative values!
        print *, ""
        print *, "CalcU error ", kA, A, Ai," ; ", kB, B, Bj," ; ", cat
        write(*,'(f9.2, "; ", 3f9.2)') LL, CLL(-A,kA) + CLL(-B, kB), CLL(-A,kA), CLL(-B, kB)
        write (*, '(3i5, " , catA ", 20i3)') A, GA, catA(1:nA)
        write (*, '(3i5, " , catB ", 20i3)') B, GB, catB(1:nB)
        do i=1,nA   
            write(*,'(a8, " ", 4i5, " ", f8.1, " ", i4)') ID(AA(i)), Parent(AA(i),:), &
              nFS(AA(i)), catA(i), Lind(AA(i)), DoneA(i)  
        enddo
...
if (A==1549 .and. k==0 .and. SB==20) then
    write(*, '("addsib ", 3i5, ", ", a8, i3, f8.1)') k, SB, A, ID(A), Inbr, LL
    write(*, '("AncB1: ", 32i5)') AncB(1,:)
    write(*, '("AncB2: ", 32i5)') AncB(2,:)
    print *, ""
endif
...
subroutine MergeSibs(SA, SB, k, LL)   ! TODO: reinstate 'sharedpar' quick option?
use Global
implicit none
...
if (SA==5 .and. SB==24 .and. k==0) then
    write(*,'("mergeSibs: ", 2i6, " - ", i6, f8.1, 2i5)') k, SA, SB, LL, cat
    write(*,'(2i4, 3f8.1)') nS(SA,k), nS(sB,k),CLL(SA,k), CLL(SB,k),CLL(SA,k) + CLL(SB,k)
    do i=1, nS(SA, k)
        write(*,'(i3, " ", a8, 2i6, " fs", 10i5)') catA(i),  ID(SibID(i,SA,k)), Parent(SibID(i,SA,k),:), &
        nFS(SibID(i,SA,k)), FSID(1:nFS(SibID(i,SA,k)), SibID(i,SA,k))
    enddo
    print *, ""
    do i=1, nS(SB, k)
        write(*,'(i3, " ",a8, 2i6, " fs", 10i5)') catB(i), ID(SibID(i,SB,k)), Parent(SibID(i,SB,k),:), &
      nFS(SibID(i,SB,k)), FSID(1:nFS(SibID(i,SB,k)), SibID(i,SB,k))
  enddo 
  print *, ""
  write(*,'("A", 3f8.1)') CLL(SA,k), Lind(SibID(1:2,SA,k))
  write(*,'("B", 2f8.1)') CLL(SB,k), Lind(SibID(1,SB,k))
  print *, ""
endif
...
!if (kB==1 .and. SB==113 .and. A==3457) print *, "bleep 1 ", LL
if (LL /= 999) return
...
!if (kB==1 .and. SB==113 .and. A==3457) print *, "bleep 2 ", LL
...
enddo
!if (kB==1 .and. SB==113 .and. A==3457) print *, "bleep 3 ", LL
!if (kB==1 .and. SB==113 .and. A==3457) write(*,'(2i3, f9.2)') MaybeFS(1:nS(SB,kB)), LLtmp(1)
...
!AddFS(A, SB, kB, SA, kA, LL)
if (kB==1 .and. SB==0 .and. A==3013) then
    write(*, '("Add FS", 2i4, "--", 2i4, "; ", 2i4, " - ", i4, 2f8.1)') kB, SB, kA, SA, GpID(:,SB,kB), PA, LLUX, LL
    do f=1, nS(SB,kB)
        write(*,'(a8, 3i6, 3f8.1)') ID(SibID(f, SB, kB)), Parent(SibID(f, SB, kB), 3-kB), Par(f), &
        MaybeFS(f), dLL(f), SUM(PrL(:, f, 1)), SUM(PrL(:, f, 2))
    enddo
    print *, ""
endif
...
if (k==0 .and. SB==87 .and. A==185) then
    print *, "addParent ", k, SB, ID(A), LL
endif
...
if (k==0 .and. SB==30 .and. A==370) then
    write (*, '("AddGGP: ", a8, 2i5, f8.1, 3i5)') ID(A), k, SB,  LL, catG, GG, AgeDiff(GG, A) ! SUM(PrL), Lind(A),
    print *, "AncG1: ", AncG(1, 1:8)
    print *, "AncG2: ", AncG(2, 1:8)
    print *, ""
endif
                ...
                do r=1, nS(SB,kB)
                    ...
                    if (Parent(Bj,3-kB) <0 .and. CatB(r)/=1) then
                        do e=1,3
                            ...
                            enddo
                        enddo
!                        if (SUM(PrE)<3)  PrE = PrE / SUM(PrE)
                    endif

                    ! TODO: correction if catA=2 & A>0
...
if (SA==36 .and. kA==0 .and. kB==1 .and. SB==33 .and. hf==3) then  
!    if (ANY(GpID(:,SA,kA)==0)) return
    print *, "ParentHFS: ", kA, A, kB, SB, " , G: ", G, ", hf: ", hf
    write(*, '("catA ", 20i3)') catA(1:nA)
    write(*, '("catB ", 20i3)') catB
    write(*, '("DoneA ", 20i3)') DoneA(1:nA)
    write(*,'(f9.2)') LL
    print *, "AncB kB-2", AncB(kB, 2), " , AncA kA-2", AncA(kA, 2)
!    print *, "GA: ", GpID(:, SA,kA), " , GB: ", GpID(:, SB,kB)
    print *, ""
endif
...
if (sA==61 .and. kA==0 .and. SB==49 .and. kB==1) then
    write(*,'("dumGP", 4i5,"; ", 2i5, ", ", 2i5, 3f8.1)') kA, SA, kB, SB, G, GGP, LLGX, SUM(PrL)
    print *, "GP B: ", GpID(:, SB, kB), ", cat: ", catA, catB
    print *, ""
    do r=1, nS(sB,kB)
        write(*,'("LLtmp", i3, " ", a8, " ", 2f8.1, ", ", 2f8.1)') r, ID(SibiD(r,SB,kB)), &
          LLtmp(r,:), LLtmp(r,:) + Lind(SibID(r, sB, kB)) - CLL(SB,kB)
    enddo
endif
...
!            else  ! TODO? or set to 0?  
    ...
    enddo
enddo
!if (s==31 .and. k==1 .and. GpID(1,s,k)==1025) then
!    write (*, '("dumBY ", f10.3, "; ", 200f10.5)') SUM(EstBY), EstBY
!    print *, ""
! endif
...
if (ANY(EstBY /= EstBY)) then  ! NaN
    print *, "DumBY NaN! "
    write (*, '("dumBY ", 2i5, ",", 3i5, ": ", 3i5, " --", 2i5, ": ", 2i5)') k, s, &
      SibID(1:3, s,k),BYS(1:3), GpID(:,s,k), BYGP
    write (*, '(f10.3, "; ", 200f10.5)')  SUM(EstBY), EstBY
!    stop
endif
...
if (ALR /= ALR) then  ! NaN
    print *, "ALR NaN! ", A, kA, B, kB, m, focal
    stop
endif

if (kA==1 .and. A==0 .and. kB==1 .and. B==-53 .and. focal==1) then
    print *, ""
    write(*,'("ALR CAU ", 2i5," ,",2i5,": ", f8.1)') kA, A, kB, B, ALR
    do x=1,2
        write (*, '("BYLR ", i5, 200f10.2)') x, BYLR(:,x)
    enddo
    do y=1,nAgeClasses + maxAgePO -1
        write (*, '("ALRtmp ", i5, 200f10.2)') y, ALRtmp(:, y)
    enddo
    
!    print *, "mx: ", maxAgePO
!    print *, "BY A: ", BYAB(1:nS(-A,kA), 1)
!   print *, "BY B: ", BYAB(1:nS(-B, kB), 2)
endif
...
do k=1,2
    do s=1,nC(k)
        if (nS(s,k)==0) then
            print *, "nS==0 !! ", s, k, ", ", SibID(1:5, s, k), ", ", GpID(:, s, k)
            stop
        endif
        call CalcCLL(s, k)
    enddo
...
if (Lind(i) < -HUGE(0.0D0)) then
    print *, ""
    print *, "invalid Lind"
    print *, ID(i), Parent(i, 1:2)
endif

if (i==0) then
    write (*, '("Lind ", 3i5, ", ", 5i5, f8.2)') i, Parent(i,:), FSID(1:5, i), Lind(i)
endif
...
if (CLL(s,k) < -HUGE(0.0D0) .or. CLL(s,k)/=CLL(s,k)) then
    print *, "CLL = -INF!", k, s,":", nS(s,k), Id(SibID(1:nS(s,k), s, k))
    print *, "gp ", GpID(:,s,k), ", cat: ", cat
    print *, "opp ", Parent(SibID(1:nS(s,k),s,k), 3-k)
    print *, ""
    stop
endif
        ...
        if ((Genos(l,A)==-9 .and. (nFS(A)<=1 .or. B>=0)) .or. Parent(A,k)/=i) then
            ...
            do x = 1, 3
                if (B>=0 .or. nFS(A)<=1) then
                    ...
                    print *, "invalid call to ParProb! ", i, k, A, B
                    stop
                endif
                if (B>0) then
...
if ((A>0 .and. ANY(Anc(:, 2:mxA)==A)) .or. (A<0 .and. ANY(Anc(k,3:mxA)==A))) then
    print *, ""
    print *, "Own Ancestor ! ", A
    write(*, '("Anc1 ", 16i6)') Anc(1,:)
    write(*, '("Anc2 ", 16i6)') Anc(2,:)
    stop
endif
...
!                if (nS(-CurPar(k),k)==1 .and. ALL(GpID(:,-CurPar(k),k)==0)) then
!                    call calcPair(i, SibID(1, -CurPar(k),k), k, .FALSE., LLA, 3)
!                else 
 ...
 !              endif
            ...
            if (i==0) then
                write(*,'("ParLR ", a8, 2i5, 2f8.1, "; ", 7f8.1)') ID(i), m, k, LLtmp(:,k,m), LLA
            endif
    ...
    !        call intpr(" ", 1, i, 1)
        ...
        do m=1,2
            ...
            do g=1,2
                ...
                if (k==0 .and. (s==8 .or. s==5)) then                
                    write(*,'("LLA", 2i4, "; ", 2i4, 7f8.1)') k, s, m, g, LLA
                endif
        ...
        if (s==6 .and. k==0) then
            do m=1,2
                write(*,'("LLtmp", i4, 2f8.1, "; ", 2f8.1)') g, LLtmp(1,:,m), LLtmp(2,:,m)
            enddo
        endif        
...
if (A==0) then
    print *, ""
    print *, "RemoveSib 2error! ", s, k, SibID(1:5, s, k)
    stop
endif
Parent(A, k) = 0
call calcCLL(s, k)
...
integer :: i,j,k,l, minAgeRepro !, nIndLH !, dumI(4)
...
do i=1,nInd
    ...
    if (OffName(i) /= Id(i) .or. j/=i) then 
        print *, "Error! Parentage file in different order than genotype file!", OffName(i)
        stop
    endif
enddo
 close(103)
